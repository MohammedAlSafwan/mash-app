...
import { Box, Card, CardActionArea, CardContent, CardMedia, IconButton, Rating, Typography, styled } from "@mui/material"
...
export const ItemCard: React.FC<Props> = ({ id, src, name, price, rating, isInCartInit }) => {
  ...
  async function updateRating(event: React.SyntheticEvent<Element, Event>, newValue: number): Promise<void> {
    ...
    try {
      const payload = await updateItem({
        id: id,
        item: { name: name, price: price, image: src, rating: newValue, isInCart: isInCart },
      }).unwrap()
  ...
  async function addToCart(event: React.MouseEvent<HTMLDivElement, MouseEvent>): Promise<void> {
    ...
    try {
      const payload = await updateItem({
        id: id,
        item: { name: name, price: price, image: src, rating: rating, isInCart: !isInCart },
      }).unwrap()
      console.log("fulfilled", payload)
    } catch (error) {
      console.error("rejected", error)
    ...
    <>
      <Card>
        ...
        <CardActionArea className={classes.item}>
          ...
          <IconButton component="div" aria-label="addShoppingCart" onClick={event => addToCart(event)}>
